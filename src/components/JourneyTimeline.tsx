...
const JourneyCard = ({
  ...
  <motion.button
    onClick={onClick}
    className={`
      relative w-full text-left rounded-lg border px-2 py-1.5 transition-all duration-200
  ...
  >
    ...
    <p className="text-[8px] sm:text-[9px] opacity-70 leading-tight mt-0.5 truncate">
      {item.sublabel}
    </p>
    {item.badges && (
      <div className="flex gap-0.5 mt-1 flex-wrap">
        {item.badges.map((b) => (
          <span
            key={b}
            className="text-[7px] sm:text-[8px] font-bold uppercase tracking-wide px-1 py-px rounded-full bg-primary/10 text-primary"
          >
            {b}
          </span>
        ))}
...
const JourneyTimeline = () => {
  ...
  return (
    <section className="mt-12 sm:mt-16 md:mt-20">
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5">
        {filterOptions.map((opt) => (
          <button
            ...
            className={`
              px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-medium transition-all duration-200 border
              ${
                activeFilter === opt
      ...
      {/* Timeline Grid — fits in viewport, no horizontal scroll */}
      <div
        className="grid gap-px bg-border/30 rounded-xl overflow-hidden border border-border/40"
        style={{
          gridTemplateColumns: `100px repeat(${journeyPeriods.length}, 1fr)`,
        }}
      >
        {/* Header Row */}
        <div className="bg-muted/60 p-2 flex items-center justify-center" />
        {journeyPeriods.map((p) => (
          <div key={p} className="bg-muted/60 p-2 text-center">
            <span className="text-[10px] sm:text-xs font-bold text-foreground/70 tabular-nums">
              {p}
            </span>
          </div>
        ))}
        ...
        {visibleCategories.map((cat) => (
          <>
            <div
              key={`${cat}-label`}
              className="bg-background/80 p-1.5 sm:p-2 flex items-start"
            >
              <div className="flex items-center gap-1">
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${categoryDots[cat]}`} />
            ...
            {journeyPeriods.map((period) => {
              ...
              return (
                <div
                  key={`${cat}-${period}`}
                  className="bg-background/50 p-1 sm:p-1.5 min-h-[48px] flex flex-col gap-1 justify-center"
                >
